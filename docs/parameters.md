# CenterForMedicalGeneticsGhent/nf-cmgg-qdnaseq pipeline parameters

A nextflow pipeline for creating bin annotations for qDNAseq
...
| `bin_sizes` | A comma-separated list of bin sizes to create annotations from | `string` | 1000,500,50,30,15,10,5,1 |  |  |
...
| `genomes_base` | Directory / URL base for CMGG references. | `string` | None |  |  |
| `genomes_ignore` | Do not load the CMGG reference config. | `boolean` |  |  |  |
| `cmgg_config_base` | The path to the CMGG configs | `string` | /conf/ |  |  |
